...
import { getCoopOrderList, getStatusValues, coopUpdateOrderStatus } from "./../apiAdmin";
import moment from 'moment';
import Jumbotron from './../../core/Jumbotron'

const Orders = () => {
    const [orders, setOrders] = useState([]);
    const [statusValues, setStatusValues] = useState(["Accepted", "Rejected"]);

    const { user, token } = isAuthenticated();


    const loadOrders = () => {
        getCoopOrderList(user._id, token).then((data = []) => {
            if (data && data.error) {
    ...
    const handleStatusChange = (e, orderId) => {
        ...
        coopUpdateOrderStatus(user._id, token, orderId, e.target.value)
            .then((data = []) => {
                if (data && data.error) {
