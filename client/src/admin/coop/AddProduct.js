...
import { createProductByType, getCategoriesByType } from "./../apiAdmin";
...
const AddProduct = () => {
    ...
    const { user, token, store } = isAuthenticated();
    const {
        name,
        description,
        price,
    ...
    const clickSubmit = event => {
        ...
        createProductByType(store._id, token, formData, "shop").then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error });
            } else {
                setValues({
