...
import { getVouchers, createNewVoucher, deleteVoucher, getSparkleShops } from "../apiAdmin";
...
const Vouchers = () => {
  ...
  const loadShops = () => {
    getSparkleShops().then(shops => {
      console.log('the shops are ')
      console.log(shops)
      if(!shops.error)
        setshops(shops)
    })
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "areas",
      key: "areas",
      render: (text) => (text ? text : "not applied"),
    },
    {
